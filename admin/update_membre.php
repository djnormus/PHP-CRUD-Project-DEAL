...
// REQUETE RECUPERATION DES CHAMPS
$membre = $pdo->query("SELECT id_membre, pseudo, nom, prenom, telephone, email, civilite, statut, date_enregistrement FROM membre where id_membre='".$id_membre."'")->fetch();
...
if (isset($_POST['pseudo']) && isset($_POST['nom']) && isset($_POST['prenom']) && isset($_POST['telephone']) && isset($_POST['email']) && isset($_POST['statut']) && isset($_POST['civilite'])) {
    ...
    // COMPARAISON PSEUDO BDD ET PSEUDO SAISIE********
    while ($verif_db = $pseudoVerif->fetch(PDO::FETCH_ASSOC)){

        foreach ($verif_db as $result){
            if($pseudo == $result){
                $resul_verif_db = true;
            }
        }
        ...
        <h3 class="mt-5">Modifier membre :</h3>
            <div class="row">
                <div class="col-12 col-md-6 mx-auto">
                    <p> Vous êtes sur le point de modier le membre avec l'ID : <?php echo $membre["id_membre"]; ?> </p>
                    <form method="POST" class=" bg-light border border-secondary rounded mt-3 p-3 mb-5" enctype="multipart/form-data" action="">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="pseudo" name="pseudo" autofocus value="<?php echo $membre["pseudo"]; ?>">
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="tel" class="form-control" id="telephone" name="telephone" value="<?php echo $membre["telephone"]; ?>">
                        </div>

                        <div class="mb-3">
                            <label for="email" class="form-label">Email :</label> <!--         MEMORISATION DES SAISIES -->
                            <input type="email" class="form-control" id="email" name="email" value="<?php echo $membre["email"]; ?>">
                        </div>

                        <!-- CIVILITE -->
                        <label for="sexe" class="form-label">Sexe :<?php echo ' "Actuel : '.$membre["civilite"].'"'; ?></label>
                        <div class="mb-3">
                        ...
                        <!-- STATUT -->
                        <label for="statut" class="form-label">Statut :<?php echo ' "Actuel : '.$membre["statut"].'"'; ?></label>
                        <div class="mb-4">
                            <div class=" form-check form-check-inline">
